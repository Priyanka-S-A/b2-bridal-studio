...
const BillView = () => {
  ...
  const generatePDF = () => {
    ...
    if (bill.mode) doc.text(`Payment Mode: ${bill.mode}`, 15, 73);
  ...
  return (
    <div style={{ background: '#000', minHeight: '100vh' }}>
      <section className="relative" style={{ padding: '9rem 0 6rem' }}>
        <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at center top, rgba(201,162,39,0.04), transparent 60%)' }} />
        ...
        >
          {/* Bill Card */}
          <div className="glass-dark rounded-sm overflow-hidden" style={{ border: '1px solid rgba(201,162,39,0.2)' }}>
            {/* Header */}
            <div className="p-8 text-center" style={{ background: 'linear-gradient(180deg, rgba(201,162,39,0.08), transparent)', borderBottom: '1px solid rgba(201,162,39,0.1)' }}>
              <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-3" style={{ border: '1px solid rgba(201,162,39,0.4)' }}>
            ...
            {/* Bill Info */}
            <div className="p-6 flex flex-wrap gap-6 justify-between" style={{ borderBottom: '1px solid rgba(201,162,39,0.06)' }}>
              <div>
                <span className="font-cinzel text-[0.5rem] tracking-[0.2em] uppercase block mb-1" style={{ color: 'rgba(201,162,39,0.4)' }}>Bill No</span>
                <span className="font-inter text-sm" style={{ color: '#F8F5F0' }}>#{bill._id.slice(-8).toUpperCase()}</span>
              </div>
              <div>
                <span className="font-cinzel text-[0.5rem] tracking-[0.2em] uppercase block mb-1" style={{ color: 'rgba(201,162,39,0.4)' }}>Date</span>
              ...
              {bill.branch && (
                <div>
                  <span className="font-cinzel text-[0.5rem] tracking-[0.2em] uppercase block mb-1" style={{ color: 'rgba(201,162,39,0.4)' }}>Branch</span>
                  <span className="font-inter text-sm" style={{ color: '#F8F5F0' }}>{bill.branch}</span>
                </div>
              )}
              {bill.mode && (
                <div>
                  <span className="font-cinzel text-[0.5rem] tracking-[0.2em] uppercase block mb-1" style={{ color: 'rgba(201,162,39,0.4)' }}>Mode</span>
                  <span className="font-inter text-sm" style={{ color: '#F8F5F0' }}>{bill.mode}</span>
                </div>
              )}
            </div>
            ...
            {bill.customerDetails?.name && (
              <div className="px-6 py-4" style={{ borderBottom: '1px solid rgba(201,162,39,0.06)' }}>
                <span className="font-cinzel text-[0.5rem] tracking-[0.2em] uppercase block mb-2" style={{ color: 'rgba(201,162,39,0.4)' }}>Customer</span>
            ...
            <div className="px-6 py-4">
              <span className="font-cinzel text-[0.5rem] tracking-[0.2em] uppercase block mb-4" style={{ color: 'rgba(201,162,39,0.4)' }}>Items</span>
              {bill.items.map((item, i) => (
                <div key={i} className="flex justify-between py-2.5" style={{ borderBottom: i < bill.items.length - 1 ? '1px solid rgba(201,162,39,0.04)' : 'none' }}>
                  <div>
                  ...
                  <span className="font-cinzel text-sm" style={{ color: '#C9A227' }}>₹{(item.price * (item.quantity || 1)).toLocaleString()}</span>
                </div>
              ))}
            </div>
