...
const renderContent = (content) => {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('## ')) {
      ...
      elements.push(
        <h3 key={i} className="font-playfair font-semibold mt-8 mb-3" style={{ fontSize: '1.1rem', color: '#C9A227' }}>
          {line.slice(4)}
        </h3>
      );
    } else if (line === '---') {
        ...
        <ul key={`ul-${i}`} className="mb-6 ml-1" style={{ listStyle: 'none' }}>
          {items.map((item, j) => (
            <li key={j} className="flex items-start gap-3 mb-2 font-inter text-sm leading-relaxed" style={{ color: 'rgba(248,245,240,0.65)' }}>
              <span className="mt-1.5 w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: '#C9A227' }} />
              <span dangerouslySetInnerHTML={{ __html: item.replace(/\*\*(.*?)\*\*/g, '<strong style="color:#e8d17a">$1</strong>') }} />
            </li>
          ))}
        </ul>
      );
        ...
        <ol key={`ol-${i}`} className="mb-6 ml-1 counter-reset-custom">
          {items.map((item, j) => (
            <li key={j} className="flex items-start gap-3 mb-3 font-inter text-sm leading-relaxed" style={{ color: 'rgba(248,245,240,0.65)' }}>
              <span className="font-cinzel text-xs mt-0.5 flex-shrink-0" style={{ color: '#C9A227', minWidth: '1.5rem' }}>{j + 1}.</span>
              <span dangerouslySetInnerHTML={{ __html: item.replace(/\*\*(.*?)\*\*/g, '<strong style="color:#e8d17a">$1</strong>') }} />
            </li>
          ))}
        </ol>
      ...
      elements.push(
        <p key={i} className="font-inter text-sm font-semibold mb-2" style={{ color: '#e8d17a' }}>
          {line.slice(2, -2)}
        </p>
      );
    } else if (line.trim() === '') {
      ...
      elements.push(
        <p key={i} className="font-cormorant italic text-base mb-4" style={{ color: 'rgba(201,162,39,0.6)' }}>
          {line.slice(1, -1)}
        </p>
      );
    } else {
      const html = line
        .replace(/\*\*(.*?)\*\*/g, '<strong style="color:#e8d17a">$1</strong>')
        .replace(/\*(.*?)\*/g, '<em>$1</em>')
    ...
    return (
      <div style={{ background: '#000', minHeight: '100vh', paddingTop: '8rem' }}>
        <div className="max-w-3xl mx-auto px-6 animate-pulse">
          <div className="h-4 rounded mb-6" style={{ background: 'rgba(201,162,39,0.08)', width: '30%' }} />
          <div className="h-8 rounded mb-4" style={{ background: 'rgba(201,162,39,0.1)' }} />
          <div className="h-4 rounded mb-8" style={{ background: 'rgba(201,162,39,0.06)', width: '50%' }} />
          <div style={{ aspectRatio: '16/9', background: 'rgba(201,162,39,0.05)', borderRadius: '2px' }} />
        </div>
      </div>
    );
  }
    ...
    <div style={{ background: '#000', minHeight: '100vh' }}>
      ...
      <motion.div
        ...
        style={{
          scaleX,
          transformOrigin: '0%',
          background: 'linear-gradient(90deg, #C9A227, #e8d17a)',
        }}
      />

      {/* Hero */}
      <section className="relative overflow-hidden" style={{ padding: '9rem 0 0' }}>
        <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at center top, rgba(201,162,39,0.05), transparent 60%)' }} />
        ...
        >
          {/* Back */}
          <Link to="/blog" className="inline-flex items-center gap-2 mb-8 font-cinzel text-[0.6rem] tracking-[0.2em] uppercase transition-colors duration-300" style={{ color: 'rgba(201,162,39,0.5)', textDecoration: 'none' }}
          ...
          <div className="flex items-center gap-3 mb-4">
            <span className="font-cinzel text-[0.55rem] tracking-[0.25em] uppercase px-3 py-1" style={{ background: 'rgba(201,162,39,0.15)', color: '#C9A227', border: '1px solid rgba(201,162,39,0.2)' }}>{blog.category}</span>
          ...
          <div className="flex items-center gap-4 mb-8">
            <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: 'linear-gradient(135deg, rgba(201,162,39,0.2), rgba(201,162,39,0.05))', border: '1px solid rgba(201,162,39,0.3)' }}>
              <span className="font-cinzel text-xs font-bold" style={{ color: '#C9A227' }}>{blog.author?.charAt(0)}</span>
            </div>
            <div>
        ...
        >
          <div className="overflow-hidden" style={{ border: '1px solid rgba(201,162,39,0.1)' }}>
      ...
      {blog.tags && blog.tags.length > 0 && (
        <section style={{ padding: '0 0 4rem' }}>
          <div className="max-w-3xl mx-auto px-6 lg:px-8">
            <div className="h-px mb-8" style={{ background: 'linear-gradient(90deg, transparent, rgba(201,162,39,0.2), transparent)' }} />
            <div className="flex flex-wrap gap-2">
              {blog.tags.map(tag => (
                <span key={tag} className="font-cinzel text-[0.55rem] tracking-[0.15em] uppercase px-4 py-1.5" style={{ background: 'rgba(201,162,39,0.06)', color: 'rgba(201,162,39,0.5)', border: '1px solid rgba(201,162,39,0.12)' }}>
                  #{tag}
                </span>
              ))}
            </div>
      ...
      <section className="relative" style={{ padding: '4rem 0 6rem' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="max-w-xl mx-auto px-6 text-center">
          <span className="font-cinzel text-[0.6rem] tracking-[0.4em] uppercase" style={{ color: '#C9A227' }}>Continue Reading</span>
