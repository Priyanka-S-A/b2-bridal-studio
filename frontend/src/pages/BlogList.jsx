...
const FilterPill = ({ label, active, onClick }) => (
  <button
    ...
    style={{
      background: active ? 'linear-gradient(135deg, #C9A227, #e8d17a)' : 'transparent',
      color: active ? '#000' : 'rgba(248,245,240,0.5)',
      border: `1px solid ${active ? 'transparent' : 'rgba(201,162,39,0.2)'}`,
  ...
  >
    <Link to={`/blog/${blog.slug}`} className="block">
      <div className="overflow-hidden" style={{ aspectRatio: '16/10', position: 'relative' }}>
        ...
        <div className="absolute top-4 left-4 font-cinzel text-[0.55rem] tracking-[0.25em] uppercase px-3 py-1.5" style={{ background: 'rgba(201,162,39,0.92)', color: '#000', fontWeight: 700 }}>
          {blog.category}
        </div>
      </div>
    </Link>
    <div className="flex flex-col flex-1 p-6 lg:p-7">
      <div className="flex items-center gap-3 mb-4">
        <span className="font-cinzel text-[0.55rem] tracking-[0.2em] uppercase" style={{ color: 'rgba(201,162,39,0.5)' }}>
          {new Date(blog.createdAt).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
        </span>
        <span style={{ color: 'rgba(201,162,39,0.2)' }}>·</span>
        <span className="font-cinzel text-[0.55rem] tracking-[0.2em] uppercase" style={{ color: 'rgba(201,162,39,0.5)' }}>{blog.readTime}</span>
      </div>
      ...
      <div className="flex items-center justify-between">
        <span className="font-cormorant italic text-sm" style={{ color: 'rgba(201,162,39,0.4)' }}>By {blog.author}</span>
        <Link to={`/blog/${blog.slug}`} className="flex items-center gap-2 font-cinzel text-[0.6rem] tracking-[0.2em] uppercase transition-all duration-300 group-hover:gap-3" style={{ color: '#C9A227', textDecoration: 'none' }}>
          Read More
          <svg width="14" height="10" viewBox="0 0 14 10" fill="none" stroke="#C9A227" strokeWidth="1.2" strokeLinecap="round"><path d="M1 5h12M7 1l6 4-6 4"/></svg>
        </Link>
      </div>
    </div>
  </motion.article>
...
const FeaturedBlog = ({ blog }) => (
  <motion.article initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }} className="group">
    <Link to={`/blog/${blog.slug}`} className="grid grid-cols-1 lg:grid-cols-2 gap-0 overflow-hidden" style={{ border: '1px solid rgba(201,162,39,0.15)', background: 'rgba(255,255,255,0.02)', textDecoration: 'none' }}>
      <div className="overflow-hidden" style={{ minHeight: '340px', position: 'relative' }}>
        ...
        <div className="absolute top-6 left-6 font-cinzel text-[0.55rem] tracking-[0.25em] uppercase px-4 py-2" style={{ background: 'rgba(201,162,39,0.92)', color: '#000', fontWeight: 700 }}>★ Featured</div>
      </div>
      <div className="flex flex-col justify-center p-8 lg:p-12">
        <div className="flex items-center gap-3 mb-3">
          <span className="font-cinzel text-[0.55rem] tracking-[0.25em] uppercase" style={{ color: '#C9A227' }}>{blog.category}</span>
          <span style={{ color: 'rgba(201,162,39,0.2)' }}>·</span>
          <span className="font-cinzel text-[0.55rem] tracking-[0.2em] uppercase" style={{ color: 'rgba(201,162,39,0.5)' }}>{blog.readTime}</span>
        </div>
        ...
        <div className="flex items-center gap-2 font-cinzel text-[0.65rem] tracking-[0.2em] uppercase transition-all duration-300 group-hover:gap-4" style={{ color: '#C9A227' }}>
          Read Full Article
          <svg width="16" height="10" viewBox="0 0 14 10" fill="none" stroke="#C9A227" strokeWidth="1.2" strokeLinecap="round"><path d="M1 5h12M7 1l6 4-6 4"/></svg>
        </div>
      </div>
    </Link>
  </motion.article>
    ...
    <div style={{ background: '#000', minHeight: '100vh' }}>
      ...
      <section className="relative overflow-hidden" style={{ padding: '10rem 0 5rem' }}>
        ...
        <div className="absolute top-24 right-12 opacity-[0.04] pointer-events-none hidden lg:block">
          <span className="font-signature" style={{ fontSize: '12rem', color: '#C9A227' }}>Blog</span>
        </div>
        <motion.div ref={headerRef} variants={staggerContainer} initial="hidden" animate={headerInView ? 'visible' : 'hidden'} className="max-w-[1300px] mx-auto px-6 lg:px-12 text-center">
          <motion.div variants={fadeUp} className="flex items-center justify-center gap-4 mb-5">
            ...
            <span className="font-cinzel text-[0.6rem] tracking-[0.5em] uppercase" style={{ color: '#C9A227' }}>Journal & Insights</span>
        ...
        <div className="max-w-[1300px] mx-auto px-6 lg:px-12">
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {[1,2,3].map(i => (
                <div key={i} className="animate-pulse" style={{ border: '1px solid rgba(201,162,39,0.08)' }}>
                  <div style={{ aspectRatio: '16/10', background: 'rgba(201,162,39,0.05)' }} />
                  <div className="p-6"><div className="h-5 rounded mb-3" style={{ background: 'rgba(201,162,39,0.08)' }} /><div className="h-3 rounded" style={{ background: 'rgba(201,162,39,0.05)', width: '70%' }} /></div>
                </div>
              ))}
            </div>
          ) : (
      ...
      <section className="relative" style={{ padding: '5rem 0 6rem' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="max-w-xl mx-auto px-6 text-center">
          <span className="font-cinzel text-[0.6rem] tracking-[0.4em] uppercase" style={{ color: '#C9A227' }}>Stay Inspired</span>
