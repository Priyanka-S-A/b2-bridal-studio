    ...
    <div style={{ background: '#000', minHeight: '100vh' }}>
      ...
      <section className="relative overflow-hidden" style={{ padding: '9rem 0 4rem' }}>
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="max-w-3xl mx-auto px-6 text-center">
          <motion.div variants={fadeUp} className="flex items-center justify-center gap-4 mb-4">
            ...
            <span className="font-cinzel text-[0.6rem] tracking-[0.5em] uppercase" style={{ color: '#C9A227' }}>Step 2 of 2</span>
      ...
      <section style={{ padding: '0 0 6rem' }}>
        <div className="max-w-4xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          >
            <h2 className="font-cinzel text-xs tracking-[0.2em] uppercase mb-6 pb-4" style={{ color: '#C9A227', borderBottom: '1px solid rgba(201,162,39,0.1)' }}>Booking Details</h2>
            ...
            <div className="flex flex-col gap-5">
              ...
              {/* Phone */}
              <div>
                <label className="block font-cinzel text-[0.55rem] tracking-[0.2em] uppercase mb-2" style={{ color: 'rgba(201,162,39,0.5)' }}>WhatsApp Number *</label>
              ...
              {/* UPI ID */}
              <div>
                <label className="block font-cinzel text-[0.55rem] tracking-[0.2em] uppercase mb-2" style={{ color: 'rgba(201,162,39,0.5)' }}>Your UPI ID *</label>
              ...
              {/* Transaction ID */}
              <div>
                <label className="block font-cinzel text-[0.55rem] tracking-[0.2em] uppercase mb-2" style={{ color: 'rgba(201,162,39,0.5)' }}>Transaction ID *</label>
              ...
              {/* Branch */}
              <div>
                <label className="block font-cinzel text-[0.55rem] tracking-[0.2em] uppercase mb-2" style={{ color: 'rgba(201,162,39,0.5)' }}>Branch *</label>
              ...
              {/* Payment Proof */}
              <div>
                <label className="block font-cinzel text-[0.55rem] tracking-[0.2em] uppercase mb-2" style={{ color: 'rgba(201,162,39,0.5)' }}>Payment Proof (Screenshot/PDF) *</label>
                <div className="relative">
                  <input
                    ...
                    style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(201,162,39,0.2)', color: '#F8F5F0' }}
                  />
                </div>
                {paymentProof && (
                  <span className="text-xs mt-1 block" style={{ color: 'rgba(201,162,39,0.5)' }}>✓ {paymentProof.name}</span>
                )}
          ...
          <motion.div
            ...
            style={{ border: '1px solid rgba(201,162,39,0.15)' }}
          >
            <h3 className="font-cinzel text-xs tracking-[0.2em] uppercase mb-5 pb-3" style={{ color: '#C9A227', borderBottom: '1px solid rgba(201,162,39,0.1)' }}>Cart Summary</h3>
            ...
            <div className="flex flex-col gap-3 mb-5">
              {items.map(item => (
                <div key={item._id || item.id} className="flex justify-between text-sm">
                  ...
                  <span className="font-cinzel text-xs" style={{ color: '#C9A227' }}>₹{(item.price * item.quantity).toLocaleString()}</span>
                </div>
              ))}
            </div>

            <div className="pt-4 flex flex-col gap-2" style={{ borderTop: '1px solid rgba(201,162,39,0.1)' }}>
              <div className="flex justify-between font-cinzel text-sm" style={{ color: '#F8F5F0' }}>
                <span>Total</span>
                <span style={{ color: '#C9A227' }}>₹{total.toFixed(2)}</span>
              </div>
            </div>

            <div className="mt-5 p-3 rounded-sm" style={{ background: 'rgba(201,162,39,0.05)', border: '1px solid rgba(201,162,39,0.08)' }}>
