      ...
      <div className="page-hero">
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="max-w-3xl mx-auto">
          <motion.div variants={fadeUp} className="flex items-center justify-center gap-4 mb-4">
            ...
            <span className="font-cinzel text-[0.65rem] tracking-[0.4em] uppercase" style={{ color: '#C9A227' }}>Portfolio</span>
            <div className="gold-divider" style={{ width: '40px' }} />
          </motion.div>
          ...
          {GALLERY_TABS.map(tab => (
            <button
              ...
              style={{
                ...
                border: `1px solid ${activeTab === tab ? 'rgba(201,162,39,0.5)' : 'rgba(201,162,39,0.12)'}`,
                color: activeTab === tab ? '#C9A227' : 'rgba(248,245,240,0.5)',
              }}
            >
              {tab}
            </button>
          ...
          <AnimatePresence mode="popLayout">
            {filtered.map((item, i) => (
              <motion.div
                ...
                style={{ aspectRatio: '1/1', border: '1px solid rgba(201,162,39,0.08)' }}
                onClick={() => setLightbox(item)}
              >
                <img src={item.src} alt={item.title} className="w-full h-full object-cover" loading="lazy" />
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col items-center justify-center" style={{ background: 'linear-gradient(to top, rgba(0,0,0,0.85), rgba(0,0,0,0.3), rgba(201,162,39,0.05))' }}>
                  <span className="font-cinzel text-[0.5rem] tracking-[0.25em] uppercase px-2 py-0.5 mb-1" style={{ background: 'rgba(201,162,39,0.9)', color: '#000' }}>{item.category}</span>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="lightbox-overlay" onClick={() => setLightbox(null)}>
            <motion.div initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.85, opacity: 0 }} transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }} className="relative max-w-4xl max-h-[85vh] mx-4" onClick={e => e.stopPropagation()}>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 text-center" style={{ background: 'linear-gradient(to top, rgba(0,0,0,0.9), transparent)' }}>
                <span className="font-cinzel text-[0.6rem] tracking-[0.25em] uppercase" style={{ color: '#C9A227' }}>{lightbox.category}</span>
              ...
              <button onClick={() => setLightbox(null)} className="absolute top-3 right-3 w-8 h-8 rounded-full flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.7)', color: '#C9A227', border: '1px solid rgba(201,162,39,0.3)' }}>✕</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
