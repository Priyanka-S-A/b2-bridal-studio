    ...
    <div style={{ background: '#000', minHeight: '100vh' }}>
      ...
      <section className="relative overflow-hidden" style={{ padding: '9rem 0 4rem' }}>
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="max-w-3xl mx-auto px-6 text-center">
          <motion.div variants={fadeUp} className="flex items-center justify-center gap-4 mb-4">
            ...
            <span className="font-cinzel text-[0.6rem] tracking-[0.5em] uppercase" style={{ color: '#C9A227' }}>Step 1 of 2</span>
      ...
      <section style={{ padding: '0 0 6rem' }}>
        <div className="max-w-4xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          >
            <span className="font-cinzel text-[0.55rem] tracking-[0.3em] uppercase block mb-6" style={{ color: '#C9A227' }}>Scan to Pay</span>
            ...
            {/* UPI ID */}
            <div className="mb-6">
              <span className="font-cinzel text-[0.5rem] tracking-[0.2em] uppercase block mb-2" style={{ color: 'rgba(201,162,39,0.5)' }}>UPI ID</span>
              <div className="flex items-center justify-center gap-3">
                <span className="font-inter text-base font-medium" style={{ color: '#C9A227' }}>{UPI_ID}</span>
                <button
            ...
            <div className="p-4 rounded-sm" style={{ background: 'rgba(201,162,39,0.05)', border: '1px solid rgba(201,162,39,0.1)' }}>
          ...
          <motion.div
            ...
            style={{ border: '1px solid rgba(201,162,39,0.15)' }}
          >
            <span className="font-cinzel text-[0.55rem] tracking-[0.3em] uppercase block mb-6" style={{ color: '#C9A227' }}>Order Summary</span>

            <div className="flex-1 overflow-y-auto mb-6" style={{ maxHeight: '300px' }}>
              {items.map(item => (
                <div key={item._id || item.id} className="flex justify-between items-center py-3" style={{ borderBottom: '1px solid rgba(201,162,39,0.06)' }}>
                  <div>
                    <span className="font-cormorant text-sm block" style={{ color: '#F8F5F0' }}>{item.name}</span>
                    <span className="font-cinzel text-[0.5rem] tracking-[0.1em]" style={{ color: 'rgba(201,162,39,0.4)' }}>Qty: {item.quantity}</span>
                  </div>
                  <span className="font-cinzel text-sm" style={{ color: '#C9A227' }}>₹{(item.price * item.quantity).toLocaleString()}</span>
                </div>
              ))}
            </div>
            ...
            <div className="pt-4 flex flex-col gap-2" style={{ borderTop: '1px solid rgba(201,162,39,0.1)' }}>
              ...
              <div className="flex justify-between font-cinzel text-base pt-2" style={{ borderTop: '1px solid rgba(201,162,39,0.08)', color: '#F8F5F0' }}>
                <span>Total</span><span style={{ color: '#C9A227' }}>₹{total.toFixed(2)}</span>
              </div>
            </div>
