      ...
      <div className="page-hero">
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="max-w-3xl mx-auto">
          <motion.div variants={fadeUp} className="flex items-center justify-center gap-4 mb-4">
            ...
            <span className="font-cinzel text-[0.65rem] tracking-[0.4em] uppercase" style={{ color: '#C9A227' }}>Shop</span>
            <div className="gold-divider" style={{ width: '40px' }} />
          </motion.div>
            ...
            return (
              ...
              >
                ...
                <div className="img-zoom-container relative" style={{ height: '240px' }}>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500">
                    {!inCart && (
                      <button
                        ...
                        style={{ background: 'rgba(201,162,39,0.95)', color: '#000' }}
                      >
                        Quick Add
                      </button>
                    )}
                  ...
                  {/* Category badge */}
                  <span className="absolute top-3 left-3 font-cinzel text-[0.5rem] tracking-[0.2em] uppercase px-2 py-1" style={{ background: 'rgba(0,0,0,0.7)', color: '#C9A227', border: '1px solid rgba(201,162,39,0.2)' }}>
                    {product.category}
                  </span>
                </div>
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="font-cinzel text-sm" style={{ color: '#C9A227' }}>₹{product.price?.toLocaleString()}</span>
                    <button
                      onClick={() => inCart ? openCart() : addToCart(product)}
                      className="w-8 h-8 flex items-center justify-center rounded-full transition-all duration-200"
                      style={{
                        border: `1px solid ${inCart ? '#C9A227' : 'rgba(201,162,39,0.3)'}`,
                        background: inCart ? 'rgba(201,162,39,0.15)' : 'transparent',
                        color: '#C9A227',
                      }}
                    >
                      {inCart ? (
