...
const PaymentVerification = () => {
  ...
  const handleAccept = async (id) => {
    if (!window.confirm('Accept this booking and generate bill?')) return;
    try {
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <h2 className="text-2xl font-bold text-gray-900 heading-luxury">Payment Verification</h2>
        <div className="flex gap-2">
          {['All', 'Pending', 'Successful', 'Rejected'].map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-4 py-2 text-xs font-semibold rounded-lg transition-colors ${filter === f ? 'bg-black text-gold-500' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
            >
